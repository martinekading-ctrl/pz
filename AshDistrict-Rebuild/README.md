...
0.33.2 将玩家和僵尸升级为真实蒙皮骨架动画，接入待机、走跑、持枪、挥击、受击、翻越与死亡动作，并按实际移动和判定时长调整播放速度。透明 3D 视口继续按脚底坐标合成到等距地图，原碰撞、搜索、驾驶与存档逻辑保持不变。当前 CC0 角色用于验证骨架与动作自然度，后续可在同一骨架规范下替换外观。见 `docs/skeletal-animation-v1.md`。

0.33 的战斗表现层仍负责近战前摇/命中/后摇、枪械后坐、命中特效、镜头反馈和原创近战音效；0.33.2 只替换其角色模型与动作来源。说明见 `docs/combat-presentation-v1.md`。
...
简易版的上线范围、完成比例和版本顺序见 `docs/simplified-mobile-scope-v1.md` 与 `docs/vertical-slice-roadmap-v1.md`。
...
- 当前上线范围只使用现有连续小街区和 11 栋可进入建筑。上线前停止扩图，先完成建筑差异、室内比例、碰撞、正式角色和手机性能。
- 过去的 v4 开放世界总图、道路图、设施表和数据文件保留为历史规划资料，不再代表当前开发承诺，也不计入完成度。
- 后续是否扩图只在简易版 1.0 发布并验证后决定；农业、动物、高层和完整 PZ 规模不属于本项目范围。
- 等距逻辑格：64×32 像素。
