...
**人物定型版**：暂以当前成年幸存者模型和走跑动作作为后续开发基线。跑步采用真人慢跑动捕，已修正按 W 前进时角色背向移动的问题；走跑速度、转向和持枪跑步已接入正式场景。Git 标签 `人物定型版` 固定这一版，方便以后回退。动作来源与复现方法见 `docs/MOTION_PIPELINE.md`。

0.33.3 换用正常成人比例的 65 骨骼模型，玩家采用橄榄色外套、深色长裤和背包，感染者采用灰蓝上衣和灰白肤色。重新适配走跑、蹲行、持枪、攻击等动作，并修复连续攻击动画重播。当前是简化的实机外观，不是设计图的逐像素还原；车辆仍为之前的简化模型。来源、重建方式和限制见 `docs/human-characters-v1.md`。
