...
func audit() -> void:
	...
	for clip in ["Walk","ZombieShuffle","ZombiePursuit"]:
		player.play(clip)
		...
		for side in ["l","r"]:
			...
			for i in 49:
				...
				if clip in ["ZombieShuffle","ZombiePursuit"]:
					var cycle := 1.05 if clip=="ZombiePursuit" else 0.85
					var clearance := 0.08 if clip=="ZombiePursuit" else 0.045
					...
					if phase<0.6:
						...
						assert(absf(p.z-(rest.z+cycle/0.95*(0.3-phase)))<0.002,"Support foot must counter world travel")
					assert(p.y-rest.y<clearance,"Swing foot must stay low")
				low=low.min(p)
				high=high.max(p)
